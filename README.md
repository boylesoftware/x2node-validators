...
* `['needs', propName, expectedValue]` - If the property being validated is not empty, tests if the parent container has a non-empty property named `propName`. If optional `expectedValue` paramster is provided, also makes sure that `propName` is equal to the expected value. Without `expectedValue`, uses message id `missingNeeded` with `prop` paramater providing the needed property name (the `propName` validator parameter). With `expectedValue`, uses message id `invalidNeeded` with `prop` for the property name and `expected` for the expected value.

* `['conflicts', propName, conflictingValue]` - If the property being validated is not empty, makes sure that the parent container does not have a non-empty property named `propName`. If optional `conflictingValue` paramster is provided, makes sure that if `propName` is present, it does not equal to the specified value. Without `conflictingValue`, uses message id `conflictingPresent` with `prop` paramater providing the conflicting property name (the `propName` validator parameter). With `conflictingValue`, uses message id `conflictingValue` with `prop` for the property name and `value` for the conflicting value.
