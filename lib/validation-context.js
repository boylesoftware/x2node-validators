...
class ValidationContext {
	 ...
	 */
	constructor(recordTypes, recordTypeDesc, messageResolver) {

		this._recordTypes = recordTypes;
		this._recordTypeDesc = recordTypeDesc;
		this._messageResolver = messageResolver;

		this._curPointer = patches.parseJSONPointer(recordTypeDesc, '');
	 ...
	 *
	 * @member {module:x2node-patches~PropertyPointer}
	 * @readonly
	 */
	 ...
	 * JSON pointer.
	 *
	 * @param {(string|module:x2node-patches~PropertyPointer)} ptr The pointer.
	 * @param {string} message Validation error message id in curly braces or
	 * message template.
	 * @param {Object.<string,*>} [params] Validation error message parameters.
	 */
	addErrorFor(ptr, message, params) {
		...
		let errorKey, propPtr;
		if ((typeof ptr) === 'string') {
			errorKey = ptr;
			propPtr = patches.parseJSONPointer(this._recordTypeDesc, ptr);
		} else {
			errorKey = ptr.toString();
			propPtr = ptr;
		}

		let errors = this._result[errorKey];
		if (!errors)
			this._result[errorKey] = errors = new Array();
		...
		errors.push(this._messageResolver.getMessage(
	...
	hasErrorsFor(ptr) {
		...
		const errorKey = ((typeof ptr) === 'string' ? ptr : ptr.toString());

		const errors = this._result[errorKey];
