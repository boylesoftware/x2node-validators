...
function normalize(recordTypes, recordTypeName, record, lang, validationSets) {
	...
	const ctx = new ValidationContext(
		recordTypes, recordTypeDesc, new MessageResolver(lang || '*'));
...
function normalizeChildren(ctx, container, containerObj, validationSets) {
	...
	if (container.isPolymorphObject()) {
		...
		if (((typeof subtype) !== 'string') || !container.hasProperty(subtype)) {
			ctx.addErrorFor(
				ctx.currentPointer.toString() + '/' + /*container.typePropertyName*/'$type',
				'{invalidType}');
			return;
		}
	}
	...
	for (let propName of container.allPropertyNames) {
		...
		// check if subtype
		if (propDesc.isSubtype() && (propDesc.name === subtype)) {
			normalizeChildren(
				ctx, propDesc.nestedProperties, containerObj, validationSets);
			continue;
		}
		...
		if (propDesc.isArray() && Array.isArray(value) && (value.length > 0)) {
			...
			if (propDesc.scalarValueType === 'object') {
				for (let i = 0, len = value.length; i < len; i++) {
					...
					if (((typeof elementValue) === 'object') &&
						(elementValue !== null))
						normalizeChildren(
							ctx, propDesc.nestedProperties, elementValue,
							validationSets);
		...
		} else if (propDesc.isMap() && ((typeof value) === 'object') &&
			(value !== null)) {
			...
			normalizeChildren(
				ctx, propDesc.nestedProperties, value, validationSets);
		}
