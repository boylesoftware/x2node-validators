...
exports.VALIDATION_ERROR_MESSAGES = {
	...
	'missingWhenPattern': {
		'en-US': 'Required when ${prop} matches ${pattern}.'
	},
	'missingWhenNotPattern': {
		'en-US': 'Required when ${prop} does not match ${pattern}.'
	},
	'notEmptyWhen': {
		'en-US': 'Expected to be empty with ${prop}.'
	},
	'notEmptyWhenNot': {
		'en-US': 'Expected to be empty with no ${prop}.'
	},
	'notEmptyWhenValue': {
	...
	'notEmptyWhenPattern': {
		'en-US': 'Expected to be empty when ${prop} matches ${pattern}.'
	},
	'notEmptyWhenNotPattern': {
		'en-US': 'Expected to be empty when ${prop} does not match ${pattern}.'
	}
};
 ...
 */
function hasMatchingSibling(ctx, propName, testValue) {
...
exports.VALIDATOR_DEFS = {
	...
	'requiredIf': function(params, ctx, value) {
		...
		if (isEmpty(ctx.currentPointer, ctx.currentPropDesc, value)) {
			...
			const testValue = params[2];
			if (hasMatchingSibling(ctx, propName, testValue)) {
				const hasTestValue = (testValue !== undefined);
				const testValueIsPattern = (testValue instanceof RegExp);
				ctx.addError(
					(
						hasTestValue ? (
							testValueIsPattern ? '{missingWhenPattern}'
								: '{missingWhenValue}'
						) : '{missingWhen}'
					), {
						prop: propName,
		...
		if (isEmpty(ctx.currentPointer, ctx.currentPropDesc, value)) {
			...
			if (!hasMatchingSibling(ctx, propName, testValue)) {
				...
				ctx.addError(
					(
						hasTestValue ? (
							testValueIsPattern ? '{missingWhenNotPattern}'
								: '{missingWhenNotValue}'
						) : '{missingWhenNot}'
					), {
						prop: propName,
		...
		if (!isEmpty(ctx.currentPointer, ctx.currentPropDesc, value)) {
			...
			if (hasMatchingSibling(ctx, propName, testValue)) {
				...
				ctx.addError(
					(
						hasTestValue ? (
							testValueIsPattern ? '{notEmptyWhenPattern}'
								: '{notEmptyWhenValue}'
						) : '{notEmptyWhen}'
					), {
						prop: propName,
		...
		if (!isEmpty(ctx.currentPointer, ctx.currentPropDesc, value)) {
			...
			if (!hasMatchingSibling(ctx, propName, testValue)) {
				...
				ctx.addError(
					(
						hasTestValue ? (
							testValueIsPattern ? '{notEmptyWhenNotPattern}'
								: '{notEmptyWhenNotValue}'
						) : '{notEmptyNotWhen}'
					), {
						prop: propName,
